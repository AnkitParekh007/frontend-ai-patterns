...
Agents should not mutate important data without consent.

## Why It Matters

Approval gates create a clear human-in-the-loop safety boundary.

## UX Behavior

Show risk level, affected record, exact proposed action, approve/reject buttons, and audit note.

## TypeScript Model

```ts
export interface ApprovalRequest { id: string; riskLevel: "low" | "medium" | "high"; approved: boolean | null; }
```

## Angular Implementation Idea

Use an ApprovalCardComponent that emits approve/reject events to the workflow service.
